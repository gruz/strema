...
# Find all log files
LOG_FILES=(
    "$SCRIPT_DIR/stream.log"
    "$SCRIPT_DIR/stream.log.old"
    "$SCRIPT_DIR/watchdog.log"
    "$SCRIPT_DIR/watchdog.log.old"
)

# Delete old .old files
find "$SCRIPT_DIR" -name "*.log.old" -type f -mtime +$MAX_AGE_DAYS -delete 2>/dev/null
...
if [ $TOTAL_SIZE -gt $MAX_TOTAL_SIZE ]; then
    ...
    rm -f "$SCRIPT_DIR"/*.log.old
    
    # Recalculate
    TOTAL_SIZE=0
    for file in "$SCRIPT_DIR/stream.log" "$SCRIPT_DIR/watchdog.log"; do
        if [ -f "$file" ]; then
            SIZE=$(stat -f%z "$file" 2>/dev/null || stat -c%s "$file" 2>/dev/null)
            TOTAL_SIZE=$((TOTAL_SIZE + SIZE))
        fi
