...
def analyze_logs():
    ...
    try:
        ...
        analyzer_script = PROJECT_ROOT / 'scripts' / 'analyze_logs.py'
        result = subprocess.run(
            [sys.executable, str(analyzer_script), '--log-dir', str(LOGS_DIR), '--hours', str(hours), '--format', 'json'],
            capture_output=True, text=True, timeout=30
        )
        if result.returncode != 0:
            return jsonify({'error': f'Analyzer failed: {result.stderr}'}), 500
